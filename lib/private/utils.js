 ...
 * ex. "A" -> 1, "AA" -> 27
 * @param {String|Number} ordinalAlphabet 順番を表す文字列または数値。ex. "A", "AA" | 1, "2"
...
function formatRecordByColumnLabel( recode ){
    ...
    recode.forEach( function( value, index ){
        recodeBuffer[ toOrdinalAlphabet( index + 1 ) ] = value;
    } );
    return recodeBuffer;
}
...
function formatRecordByMapping( recode, mapping ){
    ...
    _( mapping ).forEach( function( position, keyName ){
        recodeBuffer[ keyName ] = recode[ toOrdinalNumber( position ) - 1 ];
    } );
    return recodeBuffer;
}
