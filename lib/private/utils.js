 ...
 */
var toOrdinalNumber = _.memoize( function toOrdinalNumber( ordinalAlphabet ){

    var intFromArg = parseInt( ordinalAlphabet, 10 );
 ...
 * @constructor
 */
var Ordinal = function( ordinalData ){
    if( !ordinalData || !/^([0-9]+|[a-zA-z]+)$/.test( ordinalData ) || ordinalData <= 0 ){
        throw new TypeError( ordinalData );
    }

    if( !( this instanceof Ordinal ) ){
        return new Ordinal( ordinalData );
    }
...
Ordinal.prototype = {
    ...
    /** @returns {Number} */
    toNumber: function(){ return toOrdinalNumber( this.ordinalData ); },

    /** @returns {String} */
    toAlphabet: function(){ return toOrdinalAlphabet( this.ordinalData ); }
};
...
function formatRecordByColumnLabel( recode ){
    ...
    recode.forEach( function( value, index ){
        recodeBuffer[ Ordinal( index + 1 ).toAlphabet() ] = value;
    } );
    return recodeBuffer;
}
...
function formatRecordByMapping( recode, mapping ){
    var recodeBuffer = {};
    _.forEach( mapping, function( position, keyName ){
        if( position === 0 ){
            return;
        }
        recodeBuffer[ keyName ] = recode[ Ordinal( position ).toNumber() - 1 ];
    } );
    return recodeBuffer;
}
